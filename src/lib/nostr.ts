...
export const commentEvent = (comment: string, post_id: string, public_key: string, private_key: string, replying_to = "" ) => {
    if (replying_to) {
        return genericEvent(4, comment, public_key, private_key, [["post",post_id], ["replying_to", replying_to]])
    } else {
        return genericEvent(4, comment, public_key, private_key, [["post",post_id]])
    }
}
